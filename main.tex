...
\Procedure{SurvivalProbability}{copula} \Comment{copula is exogenously inputted.}
    ...
    \State true\_res\_mat\_s $\gets \Bar{C}(\boldsymbol{1} - \boldsymbol{u\_mat})$ \Comment{obtain exceedance probability for true copula}
    \For{$b$ in 1:$B$}
        \State $\boldsymbol{U}^{n,d} \gets$ sampling from copula \Comment{See Algorithm \ref{GaussianCopulaResampling}, \ref{tCopulaResampling}, \ref{MarshallOlkinAlgo}}
...
\Procedure{CumulativeProbability}{copula} \Comment{copula is exogenously inputted.}
    ...
    \State true\_res\_mat\_c $\gets C(\boldsymbol{u\_mat})$ \Comment{obtain cumulative probability for true copula}
    \For{$b$ in 1:$B$}
        \State $\boldsymbol{U}^{n,d} \gets$ sampling from copula \Comment{See Algorithm \ref{GaussianCopulaResampling}, \ref{tCopulaResampling}, \ref{MarshallOlkinAlgo}}
        \State ec\_res\_mat\_c($b$) $\gets \frac{1}{n} \sum\limits_{i = 1}^{j} \mathds{1}\{\Hat{\boldsymbol{U}}_{i} \le \boldsymbol{u\_mat}_{i,\cdot}\}$ \Comment{$\Hat{\boldsymbol{U}}$ and $\boldsymbol{R}$ defined previously}
