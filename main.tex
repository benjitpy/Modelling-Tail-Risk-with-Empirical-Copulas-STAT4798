...
\begin{algorithmic}
\Procedure{SurvivalFunctionEC}{$\mathbf{u}$, $\mathbf{U}$, copula}
    ...
    \For{i in 1:n}
        \State $ \Bar{H}(\textbf{u}_{i,\cdot}) \gets \frac{1}{n} \sum\limits_{i = 1}^{n} \mathds{1}\{\mathbf{\Hat{U}}_{i,\cdot} \ge \mathbf{u}_{i,\cdot}\}$ \Comment{Analysed componentwise}
    \EndFor
...
\Procedure{SurvivalProbability}{copula} \Comment{copula is exogenously inputted.}
    ...
    \State true\_res\_mat $\gets \Bar{C}(1 - \textbf{u})$ \Comment{obtain exceedance probability for true copula}
    \For{b in 1:B}
